import { getCurrentRange, getPassRange } from 'common/field/helper';

export const timeOptions = [
  { label: '今日', value: getCurrentRange('day') },
  { label: '最近3天', value: getPassRange(3, 'day') },
  { label: '最近1周', value: getPassRange(1, 'week') },
];

export const orderStatusOptions = [
  // { label: '待兑换', value: 'UNPAID', type: 'info' },
  { label: '待成团', value: 'WAITING_GROUP', type: 'info' },
  { label: '备货中', value: 'STOCK_IN_PREPARATION', type: 'warning' },
  { label: '待自提', value: 'WAITING_SELF_PICK', type: 'info' },
  { label: '已完成', value: 'CONFIRMED', type: 'success' },
  // { label: '待发放', value: 'UNISSUE', type: 'info' },
  { label: '已取消', value: 'CANCELLED', type: 'disabled' },
  { label: '已关闭', value: 'CLOSED', type: 'disabled' },
];

export const orderStatusMap = orderStatusOptions.reduce((current, item) => ({ ...current, [item.value]: item }), {});

export const orderChannelOptions = [
  { label: '微信小程序', value: 'WXAPP' },
  { label: '支付宝小程序', value: 'ALIAPP' },
];

export const campaignTagOptions = [
  { label: '秒杀', value: 'SECKILL' },
  { label: '拼团', value: 'GROUP_PURCHASING' },
  { label: '非活动', value: 'NONE' },
];

export const campaignTagMap = campaignTagOptions.reduce(
  (current, item) => ({ ...current, [item.value]: item.label }),
  {}
);

export const requiredStatusOptions = [
  { label: '未要货', value: 'UNREQUIRED' },
  { label: '待审核', value: 'AUDITING' },
  { label: '已取消', value: 'CANCELED' },
];

export const settlementStatusOptions = [
  { label: '未结算', value: 'UNSETTLEMENT' },
  { label: '待结算', value: 'SETTLEMENTING' },
  { label: '已结算', value: 'SETTLEMENTED' },
];

export const settlementTypeOptions = [
  { label: '费用补偿', value: 'COSTCOMPENSATIOB' },
  { label: '货物补偿', value: 'GOODSCOMPENSATION' },
  { label: '无需补偿', value: 'NOTCOMPENSATION' },
];

export const filterFields = [
  {
    title: '下单时间',
    name: 'createTimeRange',
    type: 'dateRangePicker',
    customProps: { options: timeOptions },
  },
  {
    title: '订单状态',
    name: 'orderStatus',
    type: 'select',
    customProps: { options: orderStatusOptions },
  },
  {
    title: '订单渠道',
    name: 'orderChannel',
    type: 'select',
    customProps: { options: orderChannelOptions },
  },
  {
    title: '活动标记',
    name: 'campaignTag',
    type: 'select',
    customProps: { options: campaignTagOptions },
  },
  {
    title: '要货状态',
    name: 'requiredStatus',
    type: 'select',
    customProps: { options: requiredStatusOptions },
  },
  {
    title: '结算状态',
    name: 'settlementStatus',
    type: 'select',
    customProps: { options: settlementStatusOptions },
  },
  {
    title: '结算方式',
    name: 'settlementType',
    type: 'select',
    customProps: { options: settlementTypeOptions },
  },
];

export const exchangeTypeMap = {
  COUPON_EXCHANGE: '券兑换',
  ENTITY_EXCHANGE: '实物兑换',
};
