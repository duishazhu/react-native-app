...
function useCompLayout(ref) {
  const [location, setLocation] = useState({ x: 0, y: 0, width: 0, height: 0, pageX: 0, pageY: 0 });
  ...
  useEffect(() => {
    if (ref.current) {
      if (isNative || isWxMp) {
        ...
        setTimeout(() =>
          ref.current.measure((x, y, width, height, pageX, pageY) => setLocation({ x, y, width, height, pageX, pageY }))
        );
      } else {
